    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip">
          ...
          <div style={{ color: "#666568"}}> Usage: {` ${payload[0].value}`} </div> 
...
export default function ReAreaChart() {
  ...
  data.aggregations.hostnames.buckets.map(function(xbucket) 
        {
            return xbucket.key;
        }
  ...
  var chart_data_values = 
    ...
    data.aggregations.hostnames.buckets.map(function(ybucket) 
            {
    ...
    return (
        <div style={{ width: '100%', height: 400 }}>
            <ResponsiveContainer>
                ...
                <XAxis dataKey="labels" interval={0}  angle={-30} height={150} dy={30}  dx={0} />
                <YAxis dataKey= "values" type="number" domain={[0, 'dataMax + 1000']} scale="linear" />
                <Tooltip content={<CustomTooltip data={chart_data} />} />
                <Area type="monotone" dataKey="values"  stroke="#8142FF" fillOpacity={1} fill="url(#colorValues)" />
